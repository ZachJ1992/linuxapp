...
char* auth_info = "MyMqttTest0120180320";//"{\"MQTT\":\"MyMqttTest0220180320\"}"; //authoriz info
...
static int MqttSample_CmdConnect(struct MqttSampleContext *ctx)
{
    ...
    //将之前创建的socket连接（ctx->mqttfd）添加到监听事件，并且监听性质设置为输入、只监听一次事件和边沿触发模式。
    //注意：将socket连接设置为输入监听事件方便了网络方面的接收处理，只需要监听网络事件就可以确定网络接收的主要情况，而不需要另外设计网络接收的等待机制。
...
static int MqttSample_Init(struct MqttSampleContext *ctx)
{
    ...
    //将标准输入stdin添加到监听事件。注意：只有当stdin有输入事件时，函数epoll_wait才会返回对应事件的数量。
